...
describe('generateOpenapiClient', () => {
	...
	test('package.json declares hey-api + tanstack deps and the codegen script', () => {
		...
		expect(pkg.scripts['codegen:check']).toContain('mktemp -d')
		expect(pkg.scripts['codegen:check']).toContain('diff -ru "$tmp/generated" src/generated')
		expect(pkg.scripts['codegen:check']).toContain("trap 'rm -rf src/generated;")
