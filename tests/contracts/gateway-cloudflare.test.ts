...
const CLOUDFLARE_TYPEGEN_SCRIPT = `wrangler types --include-runtime=false && rm -f ${CLOUDFLARE_TYPES_BOOTSTRAP_FILE}`
...
describe('Cloudflare gateway production topology', () => {
	...
	for (const [fixture, webHost] of [
		...
		test(`${fixture} keeps services private and exposes only the intended bindings`, () => {
			...
			expect(web.services).toEqual([
				{
					binding: 'GATEWAY',
					service: `${fixture === 'astro-cf-workers-full' ? 'astro-cf-workers-full' : 'hono-cf-workers-passwordless'}-api`
				}
			])
					...
					expect(service.d1_databases).toEqual([
						{
							...
							database_id: '<run: wrangler d1 create hono-cf-workers-passwordless-db>'
						}
					])
	...
	test('Wrangler can replace clean-install bootstrap types at its default path', () => {
		...
		for (const directory of workers) {
			...
			expect(bootstrap).toContain(
				'/* eslint-disable -- pnpm cf-typegen replaces these bootstrap declarations */'
			)
			expect(bootstrap).toContain('/// <reference types="@cloudflare/workers-types" />')
			expect(bootstrap).not.toContain('Bootstrap bindings derived from wrangler.jsonc')
			expect(bootstrap).not.toContain('Run pnpm cf-typegen')
			expect(bootstrap).toContain('interface Env extends __BaseEnv_Env')
			for (const marker of WRANGLER_4_125_OWNERSHIP_MARKERS) expect(bootstrap).not.toContain(marker)
		}
		...
		expect(entry(entries, `apps/api/${CLOUDFLARE_TYPES_BOOTSTRAP_FILE}`)).toContain('AUTH: Fetcher')
	...
	test('environment examples contain no direct-service public URLs', () => {
		...
		expect(production).toContain('GATEWAY_PUBLIC_ORIGINS=https://<domain>,https://api.<domain>')
	...
	test('plain local development overlays scoped root values and shares D1 state', () => {
		...
		expect(authPackage.scripts.dev).toContain('wrangler dev --persist-to ../../.wrangler/state')
		expect(usersPackage.scripts.dev).toContain('wrangler dev --persist-to ../../.wrangler/state')
		expect(dbPackage.scripts['db:prepare:local']).toBe(
			'drizzle-kit generate && pnpm db:migrate:local'
		)
		expect(dbPackage.scripts['db:migrate:local']).toContain('--persist-to ../../.wrangler/state')
