	...
	return output
		...
		.replace(
			...
			(_match, prefix: string, value: string) =>
				/^(?:\[REDACTED\]|\$\{|<|process\.env|(?:[A-Za-z_$][A-Za-z0-9_$]*\.)?env\.|string\b)/.test(value)
					? `${prefix}${value}`
					: `${prefix}[REDACTED]`
		)
}
...
export function unsafeArtifactFindings(input: string): string[] {
	const checks = [
		['absolute machine path', /(?:\/Users\/(?!\[REDACTED\])|\/home\/(?!\[REDACTED\])|[A-Z]:\\Users\\(?!\[REDACTED\]))/],
		['private key', /-----BEGIN (?:RSA |EC |OPENSSH )?PRIVATE KEY-----/],
		['bearer token', /Authorization\s*:\s*Bearer\s+(?!\[REDACTED\]|\$[A-Z_])\S+/i],
		[
			'credential URL',
			/postgres(?:ql)?:\/\/(?!\[REDACTED\]|user:pass@localhost)[^\s"'`]+/i
		],
		['credential-shaped token', /(?:AKIA[0-9A-Z]{16}|gh[opusr]_[A-Za-z0-9_]{20,}|npm_[A-Za-z0-9]{20,}|sk_live_[A-Za-z0-9]{12,}|eyJ[A-Za-z0-9_-]{20,}\.[A-Za-z0-9_-]{10,}\.)/],
		[
