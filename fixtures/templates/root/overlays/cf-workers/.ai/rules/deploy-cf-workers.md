...
Wrangler generates `worker-configuration.d.ts` from `wrangler.jsonc`; never edit
that generated file. Each `apps/api/<service>/env.d.ts` is a hand-edited,
service-owned augmentation for its local bindings and secrets. TypeScript
interface declaration merging combines the generated base and the service
augmentation.
<!--@gvkit:else-->
...
<!--@gvkit:if effectBackend-->
`apps/web/src/app.d.ts` references the generated `Env`. API services declare
their service-owned additions only in `env.d.ts`; keep those augmentations
local and rerun `wrangler types` after changing bindings.
<!--@gvkit:else-->
...
<!--@gvkit:if effectBackend-->
| Edit `worker-configuration.d.ts` | Edit the owning service's `env.d.ts`, then run `wrangler types` |
<!--@gvkit:else-->
